...
plotBmatrixCI <- function(modelFile){
  ...
  ndxStrmSea <- grep("B.\\(X.stmrCMS,",coefNames)
  ndxPrecSea <- grep("B.\\(X.precCMS,",coefNames)
  ndxEvapSea <- grep("B.\\(X.evapCMS,",coefNames)
  ndxrOffSea <- grep("B.\\(X.rOffCMS,",coefNames)
  ndxdStoSea <- grep("B.\\(X.dStoCMS,",coefNames)
  ...
  dfParm_B <- data.frame(
    cbind(as.numeric(rep(1,5)),
          ...
          as.numeric(modelFile$par.se$B),
          as.numeric(modelFile$par.se$B),
          1.96*as.numeric(modelFile$par.se$B)))
  ...
  p_aug <- p +
    xlab("Component") + ylab("Lag 1 Autocorrelation Coefficient") +
    ggtitle("Expected Magnitude and 95% Confidence Intervals for Lag 1 Autoregression in Component")
