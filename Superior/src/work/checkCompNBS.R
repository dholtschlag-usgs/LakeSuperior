...
# Create dataframe for prec
nbs_Df       <- cbind.data.frame(nbs_DateSeq,nbs_CMS)
# Plot monthly runoff time series
plot(nbs_DateSeq,nbs_CMS,type="l",
...
# Merge nbs_CMS in
SupCompNBS      <- merge(precEvaprOffDf, nbs_Df, by.x = "DateSeq", by.y = "nbs_DateSeq")
rm(list=c("precDf","evapDf","rOffDf","nbs_Df","precEvapDf","precEvaprOffDf"))
#
# Compute components estimate of net basin supply
SupCompNBS$nbsxCMS <- SupCompNBS$prec - SupCompNBS$evap + SupCompNBS$rOff
with(SupCompNBS, plot(nbs_CMS,nbsxCMS,pch=20,cex=0.75,col="tan",
