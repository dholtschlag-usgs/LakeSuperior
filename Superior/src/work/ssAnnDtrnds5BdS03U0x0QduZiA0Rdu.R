...
for (i in 1:nSample){
  ...
  ssAnnDtrnds5Bds1U0x0QduZiA0Rdu <- tryCatch(
{
  MARSS(t(dtrndMatrix), method="BFGS",
        model = model.lst,
        inits = ssAnnDtrnds5Bds1U0x0QduZiA0Rdu$par)
},
error     =  function(cond) {
  message(" * * * An error was encountered.  Returning NA * * *")
  return(NA)
}
  )
...
save(AICc,converge,parmB,logLik,Qest,Rest,numIter,file="ssAnnDtrnds5Bds03U0x0QduZiA0Rdu.RData")