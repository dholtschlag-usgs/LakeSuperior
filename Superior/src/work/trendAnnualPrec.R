...
precCMSyr <- aggregate(NBSrcDf$precCMS,
                       by=list(format(NBSrcDf$DateSeq,'%Y')),
                       FUN=mean)

precCMSyr$Year <- as.numeric(precCMSyr$Group.1)

library(MARSS)

par(mar=c(5,4,2,2)+0.1)
plot(precCMSyr$Year,precCMSyr$x,
     ylab="Annual Precipitation, in CMS",
     main="Annual Overlake Precipitation on Lake Superior",
...
lines(precCMSyr$Year,kf0$xtT,lty="dashed",col="red")
...
lines(precCMSyr$Year,kf1$xtT,lty="dotdash",col="black")
...
lines(precCMSyr$Year,kf4$xtT,lty="solid",col="magenta")
...
par(mar=c(5,4,2,2))
precInno <- kfss0$Innov
denInnov <- density(precInno)
#
plot(denInnov,xlab="Precipitation Residuals from Level, in CMS",
     main="Distribution of Annual Overlake Precipitation on Lake Superior Annual from Level",
