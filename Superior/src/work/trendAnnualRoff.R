...
rOffCMSyr <- aggregate(NBSrcDf$rOffCMS,
                       by=list(format(NBSrcDf$DateSeq,'%Y')),
                       FUN=mean)

rOffCMSyr$Year <- as.numeric(rOffCMSyr$Group.1)

library(MARSS)

par(mar=c(5,4,2,2)+0.1)
plot(rOffCMSyr$Year,rOffCMSyr$x,
     ylab="Annual Basin Runoff, in CMS",
     main="Annual Basin Runoff to Lake Superior",
...
lines(rOffCMSyr$Year,kf0$xtT,lty="dashed",col="red")
...
lines(rOffCMSyr$Year,kf1$xtT,lty="dotdash",col="black")
...
lines(rOffCMSyr$Year,kf4$xtT,lty="solid",col="magenta")
...
par(mar=c(5,4,2,2))
rOffInno <- kfss4$Innov
denInnov <- density(rOffInno)
#
plot(denInnov,xlab="Basin Runoff Residuals from Step Trend 1998, in CMS",
     main="Distribution of Annual Basin Runoff to Lake Superior from Step Trend 1998",
     type = "n", xlim=c(-800, 800), ylim=c(0,2e-3), cex.main=1, cex.axis = 0.8 )
polygon(denInnov, col = "wheat")
abline(v=mean(kfss4$Innov), col="red", lty="dashed")

normDensity <- dnorm(seq(from=-800, to=800, by = 10),
                     mean = mean(kfss4$Innov),
                     sd   =   sd(kfss4$Innov))

lines(seq(from=-800, to=800, by = 10), normDensity, col="blue")
