...
for (i in 1:12){
  ...
  plot(stmrCMSmon$DateSeq,stmrCMSmon$stmrCMS,
       ylab=expression(paste("Streamflow,  ", m^3%.%s^-1)),
...
# Plot the trend in monthly Streamflow for the minimum AIC
...
plot(stmrCMSmon$DateSeq,seq(1,63),type="n",
     ...
     ylab=expression(paste("Streamflow, in  ",m^3%.% s^-1)),
     xlab="Year",
     main="Monthly Trends in Streamflow from St. Marys River to Lake Superior")
...
for (i in 1:12) {
  ...
  text(stmrCMSmon$DateSeq[4*(i-1)+2], trnd[4*(i-1)+1]+120, label=month.abb[i],
