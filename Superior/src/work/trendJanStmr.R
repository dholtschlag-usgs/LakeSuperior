...
plot(NBSrcDf$DateSeq[seq(from=1,to=756,by=12)],
     ...
     xlab="Year",
     main=paste0(month.name[as.numeric(month.chr)],
                 "Flows of St. Marys River from Lake Superior")
for (i in 2:12){
...
# Select by month
month = "01"
stmrCMSmon <- subset(NBSrcDf, Month == month)

plot(stmrCMSmon$DateSeq,stmrCMSmon$stmr,
     ylab="January Flow, in CMS",
...
lines(stmrCMSmon$DateSeq,kf1$xtT,lty="dotdash",col="black")
