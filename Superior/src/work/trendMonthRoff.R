...
for (i in 1:12){
  ...
  plot(stmrCMSmon$DateSeq,stmrCMSmon$rOffCMS,
       ylab=expression(paste("Runoff,  ", m^3%.%s^-1)),
...
# Plot the trend in monthly Runoff for the minimum AIC
...
plot(stmrCMSmon$DateSeq,seq(1,63),type="n",
     ...
     ylab=expression(paste("Runoff, in  ",m^3%.% s^-1)),
     xlab="Year",
     main="Monthly Trends in Basin Runoff to Lake Superior")
