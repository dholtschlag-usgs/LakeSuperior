...
# residual_NBS = ChangeInStorage - Inflow + Outflow

1 Read and Plot Monthly Changes in Lake Superior Storage  

```{r echo = TRUE}
#
# utils library is used with download.file construct
library(utils)
# locfit is used in density estimation constrained to data limits
library(locfit)
#
# URL for GitHub file
urlName <- "https://raw.githubusercontent.com/dholtschlag-usgs/LakeSuperior/master/Superior/data/Monthly/SupStorageCMS.txt"
# method="curl" does not work
download.file(urlName, destfile = "SupStorageCMS.txt", method = "wget")
#
df1 <- read.table("SupStorageCMS.txt",sep="\t",header=TRUE,comment.char="#")
...
2 Read and Plot Monthly Outflow from Lake Superior Through St. Marys River 

```{r}
# Read outflow data
# URL for GitHub file
urlName <- "https://raw.githubusercontent.com/dholtschlag-usgs/LakeSuperior/master/Superior/data/Monthly/SupStMarysCMS.txt"
# method="curl" does not work
download.file(urlName, destfile = "SupStMarysCMS.txt", method = "wget")
#
df1 <- read.table("SupStMarysCMS.txt",sep="\t",header=TRUE,comment.char="#")
#
mat1     <- as.matrix(df1[,2:13])
# Convert transposed matrix to vector
...
#
```
2 Read and Plot Monthly Diversions into Lake Superior from Lake Ogoki and Long Lake  

```{r}
# Read inflow diversions data
# URL for GitHub file
urlName <- "https://raw.githubusercontent.com/dholtschlag-usgs/LakeSuperior/master/Superior/data/Monthly/SupDiversionsCMS.txt"
# method="curl" does not work
download.file(urlName, destfile = "SupDiversionsCMS.txt", method = "wget")
#
df1 <- read.table("SupDiversionsCMS.txt",sep="\t",header=TRUE,comment.char="#")
#
mat1     <- as.matrix(df1[,2:13])
# Convert transposed matrix to vector
...
```
3 Read and Plot Monthly Residual Net Basin Supply for Lake Superior  

```{r}
# URL for GitHub file
urlName <- "https://raw.githubusercontent.com/dholtschlag-usgs/LakeSuperior/master/Superior/data/Monthly/SupNBSresidCMS.txt"
# method="curl" does not work
download.file(urlName, destfile = "SupNBSresidCMS.txt", method = "wget")
#
df1 <- read.table("SupNBSresidCMS.txt",sep="\t",header=TRUE,comment.char="#")
#
mat1     <- as.matrix(df1[,2:13])
# Convert transposed matrix to vector
...
#
```
4 Plot Probability Densities for Residual Elements  
...
#
```
5 Plot Monthly Boxplot for Elements of Residual Net Basin Supply  
