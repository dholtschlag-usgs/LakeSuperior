...
for (i in 1:12){
  ...
  plot(stmrCMSmon$DateSeq,stmrCMSmon$evapCMS,
...
# Plot the trend in monthly evaporation for the minimum AIC
...
plot(stmrCMSmon$DateSeq,seq(1,63),type="n",
     ...
     ylab=expression(paste("Evaporation, in  ",m^3%.% s^-1)),
     xlab="Year",
     main="Monthly Trends in Overlake Evaporation from Lake Superior")
...
for (i in 1:12) {
  ...
  text(stmrCMSmon$DateSeq[4*(i-1)+2], trnd[4*(i-1)+1]+100, label=month.abb[i],
