...
if (paramSel == "Season"){
  ...
  return(ciPlot)
} else if (paramSel == "ACF"){
  marssModel <- seas.mod.7
  compNames  <- row.names(dat)
  dimDat     <- dim(dat)
  marssResid <- t(matrix(c(unlist(residuals(marssModel)[1])),dimDat[1],dimDat[2]))
  for (j in 1:5){
    plot.ts(marssResid[,j], ylab="Residuals", 
            main=paste("Time Series of Model Residuals for Component",compNames[j]),
            pch=20,cex=0.5,col="blue")
    abline(h=0,col="red",lty="dashed")
    acf(marssResid[,j],xlab="Lag, in Months",
        main=paste("Autocorrelation Function of Model Residuals for Component",compNames[j]))
  }
} else{
